...
export default class MixerControlFacade {
    ...
    _applyChanges(key) {
        // TODO first call must be called after all values has been updated --> emit it 
    ...
    */
    ...
    _addNewDeviceEntry(currentOutputDeviceSchemaList, id) {
        ...
        // check if device is with another name there
        // currentOutputDeviceSchemaList = currentOutputDeviceSchemaList.find((o, i) => {
        //     if (o[1] === uuid) {
        //         // TODO uuid as key, then run_time_id as element
        //         // delete currentOutputDeviceSchemaList[i] = { name: 'new string', value: 'this', other: 'that' };
        //         console.log(`Found object: ${o.join(" / ")}`);
        //         // new entry for the dataset
        //         currentOutputDeviceSchemaList[id] = o;
        //         // delete old one
        //         delete currentOutputDeviceSchemaList[i];
    ...
    _updateDeviceSet(currentOutputDeviceSchemaList, dataset, oldRuntimeId, newRuntimeId) {
        console.log(`Found object: ${dataset.join(" / ")}`);
        // new entry for the dataset
    ...
    _updateSpeakerAndHeadphones(oldRuntimeId, newRuntimeId) {
        ...
        [headphoneSet, speakerSet].map( (currentSet,index) => {
            console.log(`Entering map: ${oldRuntimeId} / ${newRuntimeId}; index:${index}; set: ${currentSet.join(" / ")}`);
    ...
    _setSpeaker(speakerDataSet) {
        // TODO should be used from prefs after modularization
