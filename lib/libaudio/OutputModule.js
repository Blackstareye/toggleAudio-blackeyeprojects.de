...
import MixerControlFacade from './MixerControlFacade.js';
...
export default class OutputModule {
    ...
    _askIfReadyFunction = null;

    constructor(settingProvider, mixerControl, isReadyFunction) {
        this._settingProvider = settingProvider;
        this._mixerControl = mixerControl;
        this._askIfReadyFunction = isReadyFunction;
    ...
    addSignals(signalHandler) {
        signalHandler.push(this._mixerControl.connect("output-added", (mixerControl, id) => this._mixerDeviceAdded(id)));
        signalHandler.push(this._mixerControl.connect("output-removed", (mixerControl, id) => this._mixerDeviceRemoved(id)));
    }
 ...
 // MARK 1
     ...
     */
    _activateDevice() {
        if (!this._askIfReadyFunction) {
            console.error("Ready function not available - see logs");
            return;
        }
        let isReady = this._askIfReadyFunction();
        console.debug("IS READY STATE:" + isReady);
