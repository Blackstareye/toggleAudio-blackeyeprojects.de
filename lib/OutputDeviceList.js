...
export default class OutputDeviceList  {
    ...
    _load() {
        ...
        // console.log(this.printSchema());
        // console.log(this.printData());
        // console.log(`ITEMS: ${this._dataMap}`);
    }
    ...
    add(id, mixerUiDevice) {
        ...
        if (!hasUpdate) {
            ...
            this.storeAndNotifyUpdate(this._dataMap[id], id);
        }
    ...
    _updateDataset(identifier, new_id) {
        ...
        for (const [key, values] of Object.entries(this._dataMap)) {
            if (values.includes(identifier)) {
                ...
                this.storeAndNotifyUpdate(values, key);
                hasUpdate = true;
                break;
            }
        }
    ...
    storeAndNotifyUpdate(element, index) {
        this._store();
        this.notifyUpdate(element, index);
    }
    notifyUpdate(element, index) {
    ...
    printData() {
        return Printer.printBeautyMap(this._dataMap);
    }
